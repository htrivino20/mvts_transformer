...
def main(config):
    ...
    data_class = data_factory[config['data_class']] # defaut value: weld
    ...
    ## Train evaluation
    aggr_metrics_train_test, per_batch_test = trainer.evaluate(epoch_num=0, keep_all=True)
    metrics_train_names, metrics_train_values = zip(*aggr_metrics_train_test.items())
    training_metrics.append(list(metrics_train_values))

    ## Test evaluation

    aggr_metrics_test_test, per_batch_test = test_evaluator.evaluate(epoch_num=0, keep_all=True)
    metrics_train_names, metrics_test_values = zip(*aggr_metrics_test_test.items())
    ...
    for epoch in tqdm(range(start_epoch + 1, config["epochs"] + 1), desc='Training Epoch', leave=False):
        ...
        # Trainning metrics
        aggr_metrics_train_test, per_batch_test = trainer.evaluate(epoch_num=epoch, keep_all=True)
        metrics_names, metrics_train_values = zip(*aggr_metrics_train_test.items())
        training_metrics.append(list(metrics_train_values))
        ...
        print_str_train_acc = 'Epoch {} Training Set Summary: '.format(epoch)
        for k, v in aggr_metrics_train_test.items():
            # print_str += '{}: {:8f} | '.format(k, v)
            print_str_train_acc += str(k) + ': ' + str(v) + ' | '

        # Test metrics
        if True:
            print("***Test metrics evaluation")
            aggr_metrics_test_test, per_batch_test = test_evaluator.evaluate(epoch_num=epoch, keep_all=True)
            metrics_names, metrics_test_values = zip(*aggr_metrics_test_test.items())
            test_metrics.append(list(metrics_test_values))


        logger.info(print_str_train_acc)
    ...
    logger.info("Training values")
    logger.info("[" + '\n'.join(["[" + ", ".join([str(value) for value in metric]) + "]," for metric in training_metrics]) + "]\n")
